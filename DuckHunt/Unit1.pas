...
begin
  ...
  if CommPortDriver1.Connect=false then
  ...
  case StI of
    ...
    4: begin
      ...
      if StC[StI].x<256 then begin
        ...
        StI:=random(Length(StC));
        DogRolf:=true;
        PlaySound('Sounds\rolf.wav', 0, SND_ASYNC);
      end;
    ...
    6: begin
        ...
        if StC[StSI].y<160 then begin
          ...
          StI:=random(Length(StC));
          DogWDuck:=true;
          PlaySound('Sounds\good.wav', 0, SND_ASYNC);
        end;
...
begin
  if StpBlckC>=4 then begin
    SettingRender.Enabled:=false;
    StpShow:=false;
    nextDuck;
  end;
